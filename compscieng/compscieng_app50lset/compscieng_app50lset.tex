...
Bir dijital resimdeki bir kümeyi, grubu ortaya çıkartmak için bir teknik daha,
kesit seviyeleri kullanmak. Bu teknik her ne kadar bir eğriyi belli bir enerji
fonksiyonunu minimize ederek bir kümenin etrafında ``sarmalayan'' yılan (snake)
tekniğine görüntüsel olarak benzese de, aslında daha ilginç özellikleri olan
bir yaklaşım. Yılan tekniğinde elde olan bir eğriyi direk değiştirerek
grup etrafını sarıp tanımlamaya uğraşıyoruz. Kesit seviyeleri ile grubu
tanımlayan manipüle ettiğimiz esas formül değil, onun yansıması.
