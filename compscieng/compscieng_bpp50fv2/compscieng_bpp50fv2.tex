...
Şimdi $u(x_1,t_1)$ mesela $x_{j-1/2}$ ve $t_l$ için (zaman $l$ indisi)
$u_{j-1}^l$ diyebiliriz, $x_2$ yerine $x_{j+1/2}$, sonuncuda zamanın hala
değişken olduğu durum $u_{j+1}$ olabilir. Eğer $x$ değişken ise, zaman indisi
$t_2 = t_{l+1}$ için $u^{l}$. 

$$
\int_{x_{j-1/2}}^{x_{j+1/2}} u^{l+1} \ud x -
\int_{x_{j-1/2}}^{x_{j+1/2}} u^{l} \ud x  +
\int_{t_l}^{t_{l+1}} f(u_{j+1/2},t) \ud t  -
\int_{t_l}^{t_{l+1}} f(u_{j-1/2},t) \ud t = 0
$$
...
[3] Bayramli, {\em Sonlu Hacim (Finite Volume) Yöntemi - 1}    
