...
Simdi $\phi_t$ icin daha once buldugumuz formulu koyarsak,
...
Simdi $F$ konusuna gelelim; bu $F$'nin secilmesi kesit seviyesi yontemimize
direk etki edecektir. Sonuc olarak egriyi belli bir yonde, ve hizda ittiren
``kuvvet'' budur. $F$ resmin her noktasinda tanimli bir nevi hiz, kuvvet
alani (velocity field) olarak gorulebilir, her noktada bize $\phi$'nin
hareketinin yonunu ve buyuklugunu verir.

Simdi dusunelim, imajda gruplama yapmak istiyorsak, ve alttaki gibi bir resim
var diyelim,

\includegraphics[width=10em]{twoObj.png}

o zaman $F$'nin ortadaki objenin sınırlarına kadar yüksek olmasını ama
sınırlarda çok az hatta sıfır olmasını isteyebiliriz, değil mi? Bu dolaylı
olarak obje tanımlamayı gerçekleştirecektir, $\phi$ yüzeyi objeye gelinceye
kadar hızla ilerleyecek, ardından obje çevresine geldiği noktalarda yavaşlar,
ve yan etki olarak kesit seviyesi nesneyi sarmalamış olur, ve bu noktada
gruplamayı yapmış oluruz.

$F$'yi o zaman direk imajın kendisinden hesaplayabiliriz, ve onu bir nevi
kenar algılayıcı (edge detector) olarak görebiliriz. Eh en basit kenar
bulucu gradyan olduğuna göre imajın gradyanini almak yeterli olacaktır. $I$
imajı için $g$ gradyanı [6],

$$
g(I) = \frac{1}{1  + || \nabla I ||^2}
$$

Hepsini bir araya koyunca alttaki kod yazilabilir,
